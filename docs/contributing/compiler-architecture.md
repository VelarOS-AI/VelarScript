...
Every fragment named above is a real JavaScript file under
`packages/compiler/runtime/`, listed by `runtime/manifest.json` with the helper
names it defines and the ones it expects from a sibling fragment. The string
constants the emitter composes are generated from those files into
`packages/compiler/src/runtime-sources.generated.ts`, which is committed and
regenerated by `npm run build:packages`; `check:runtime-sources` refuses a
transcription that has drifted, and `check:boundaries` refuses a new multi-line
`String.raw` literal in `packages/compiler/src`. A shared project module is
...
capability module that bakes in a grant — the link schemes, the declared window
kinds, the served services — is cut into runtime fragments at whole lines above
