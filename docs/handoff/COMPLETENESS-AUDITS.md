...
| MOD-U3 | **【D50 第 100 条裁决为不实现 —— D38 第 49 条作废】** `import type`，今天体验被 MOD-I1 放大 | 归批次 F 不变；I1 先行独立落地 |
...
## 审计十一 —— 语法歧义与优先级（2026-08-13，双树验证，160+ 探针）

**对账本的两处修正**（双树核实）：D36 第 37 条（可选链整尾短路）**未落地**
（charter 仍是逐环显式）；D30 第 17 条（纯表达式语句拒绝）**未落地**（`x == 5`
作语句今天零诊断）。两者均在批次 F 排队 —— 账本此前误记为已落地。

### DEFECT（解析/发射产出静默错误结构）

| ID | 现象 | 处置 |
|---|---|---|
| **GRM-D1** | **嵌套 `is` 发射未加括号的 typeof 链 —— 连源码加括号都错**：`(x is number) is bool` 打印 **false**（两步拆写打印 true）。发射成 `typeof typeof (...)`，JS 先绑内层。`parenthesized` 标记只存在于 BinaryExpression、从不存在于 IsExpression | 修（N-2b）：emitIsCheck 直接路径给 Is/Unary 操作数加括号；**并按 D42 精神 analyzer 拒绝主体静态为 bool 的 `is`**（恒定测试） |
| **GRM-D2** | **`++i` 是合法的静默空操作**（解析为 `+(+i)` 表达式语句，i 不变）；`i++` 报错但消息裸 | 批次 F 的纯语句规则须显式涵盖语句头堆叠一元 `+/-`；`++i`/`i++` 定向教 `i += 1`（消息归 N-2b） |
| GRM-D3 | `007` 静默等于 7（双亲都拒绝前导零）—— D30 §18.4 已裁、批次 E 排队，钉住仍开放 | 批次 E 不变 |

### AMBIGUOUS（两种解读竞争，赢家无文档或惊人）

| ID | 现象 | 处置 |
|---|---|---|
| **GRM-A1（全审计最高风险）** | **`==`/`!=` 与混向比较静默成 Python 链**：`a == b == c`（三个 false）→ **true**（链式两两与）；JS 读法 → **false**。`1 < 2 > 1` → true（JS false）。同一拼写、双亲都合法、答案相反 —— charter 无链成员规则、无优先级表 | **D30 第 20 条（同向链限定、==/!= 永不链）批次 F 排队 —— 本审计确认紧迫性，升优先** |
| **GRM-A2** | `in` 在比较层但**不链**（JS 左结合）而比较**链**（Python）—— 同一层撕裂双亲：`1 < 2 in [true]` → true（Python 读法 False），类型不合时报错莫名其妙 | D30 §20 的诊断**扩展涵盖 `in`/`is`**（比较层节点作另一比较层操作数须加括号，教 split with 'and'）→ 批次 F |
| **GRM-A3** | **JSX vs `<` 由一张无文档的前 token 表决定，`??` 不在表上**：`{name ?? <Fallback/>}` 裸报（含义合法 —— 加括号就过），`?`/`:` 前却可以 | 修（N-2c）：`shouldStartJsx` 表加 nullish 一项 + `and`-前-JSX 定向教三元 + charter §14 发布判定规则 |
| **GRM-A4** | **`=>` 后花括号是记录、永远不是块** —— 设计正确、零文档、一个洞：`() => {}` 是**空记录工厂**不是空操作，core 上下文位能抓（消息不教）、**web on:click 处理器静默接受** | charter §7 一句 + 不匹配消息点名记录 vs 块 + web 处理器收紧（N-2c） |
| **GRM-A5** | **续行规则半文档化**：括号内换行不敏感（已实现）但 charter §2 说「只有一种续行形式」（前导 `.`）—— 规则没写，全部裸操作符断行得到泄漏内部 token 名的级联 | charter §2 一句 + 行尾操作符定向诊断（N-2b 消息 + N-3 文档） |

### TRAP（一字符之差、两种合法含义）

| ID | 陷阱 | 处置 |
|---|---|---|
| **GRM-T2（无解析级解法）** | **Python 地板除变注释**：`7 // 2 + 1` → **编译干净、= 7**（`//` 起注释、语句自然完整，守卫无从触发） | AI 简报 + charter 一行（「地板除是 `(a/b).floor()`；`//` 永远是注释」）+ 对抗语料条目（N-3） |
| GRM-T1 | `f(x=1)` 命名实参 vs `f(x==1)` 比较：守卫矩阵大体密（未知名/重复/位置后名/类型都抓），残余静默窗口窄；两处边缘缺陷 —— VEL2024 措辞读反、`f(x=y=2)` 错 span 级联 | 消息修 N-2b |
| GRM-T3/T4 | `-2.abs()` = -2（成员先于负号 —— 双亲同绑定但双亲都拒该拼写、Vel 独接受）；`-2 ** 2` = -4（Python 语义、JS 干脆禁止该形） | 优先级表文档行（N-3）；`1.abs()` 三种拼写全通且一致 —— **比双亲都好**，值得成文 |
| GRM-T6 | 数字字面量误导消息全家（`1_000`→"Unknown unit '_000'"、`0xFF`、`.5`、`5.`、f"{x:.2f}" 级联；Infinity/NaN 不可拼写 —— 由省略决定、零文档） | 批次 E 钉住 + 字面量故事成文（N-3） |

### 诊断质量缺口（竞争位置的失败者体验）

- **TS 泛型调用守卫是名字表门控的**：同文件 def 泛型得到优秀的 VEL2031，
  **导入泛型/方法/构造器形全部漏到比较链类型错误**（泄漏函数类型内部）——
  修：`explicitTypeArgumentsEnd` 恢复扩展到任意被调形（去名字表门控）→ N-2b。
- **JS/Python 运算符词汇缺教学**：`;` 裸报（charter 明文排除分号 —— 应得同等
  礼遇）、`a | b` 误导（联合拼写在类型层存在）、`& ^` 裸报、`function(){}`、
  `:=` 七连级联 → N-2b 消息批；`/* */`（D36 §40.1）与 `not x in y`（D30 §19）
  批次 F 排队钉住。
- `def f(from:)` 声明五连级联不点名关键字，而**调用侧** `from` 特判为合法标签
  （stdlib 签名在用）—— 声明/调用不对称归批次 G。

### 软关键字碰撞网格（批次 G 工单，160 探针）

核心六词（type/match/case/from/as/enum）：绑定/参数/for/命名实参位**全拒且消息
从不点名关键字**（裸 VEL2011/2001）；解构简写有定向教学；成员/字段位全通。
web 六词（state/action/look/component/resource/watch）：**core 文件全通、web
文件全拒** —— W-1 单行可移植性破坏按位置钉死。`computed` 走**不同机制**
（VEL3007 点名保留 —— 消息比兄弟们好），但 **`{computed}` 记录简写静默捕获
内建** —— 与全部兄弟不一致。`mounted` 生命周期词必须上 G 波清单。

### 推导出的运算符优先级表（**无任何文档发布过一张** —— 12 行全探针验证）

松→紧：`=>` → `?:`（右嵌套、作二元操作数必须括号）→ `??`（与 and/or 混用
VEL2034 已落地）→ `or` → `and` → **比较层**（`== != < <= > >=` Python 链 +
`is`/`in` JS 左结合不链 —— **同层撕裂双亲、全表最高风险行**）→ 二元 `+ -` →
`* / %` → 一元 `not + -`（**低于 `**`**：`-2 ** 2` = -4）→ `**`（右结合）→
`await` → 后缀 `() . ?. []`。赋值是语句非表达式（VEL2028 处处落地）。
**该表进 charter（D30 §19 的表已批、内容即此）→ 批次 F。**

### DECIDED-AND-CORRECT（压缩）

`??` 混用 VEL2034 双向落地（D32 §31 完整）；`a ?? b ? c : d` JS 一致；嵌套三元
右结合；`is ? :` 有优秀 VEL2031 括号指引；f-string 花括号 Python 一致 + 双向
定向 VEL2009；尾逗号全位置合法；`a?.b = 1` VEL3002；`a?.[0]`/`a?.()` 存在；
链求值每项一次带短路；`await x ?? y` JS 一致；`-3 % 2` = -1（JS 母，D36 §40.4
文档排队）；`match (n):` 无碍；ASI 式连接结构上不可能；哈希注释/十六进制色
VEL1005 定向。

### 处置总结

GRM-D1 + 消息批（T1/D2 定向、泛型恢复扩展、运算符词汇教学、A5 行尾诊断）→
**N-2b**；A3/A4 → **N-2c**；A1/A2 + 优先级表 + D3/T6 → **批次 E/F（升优先）**；
T2/T3/T4 + 字面量故事 + A4/A5 文档 → **N-3**；软关键字网格 + from 不对称 +
`{computed}` 简写 → **批次 G 工单**。


---

## 盲测第一轮（2026-08-13，Codex 执行，严格盲条件：只准简报 + 诊断）

**结果**：3/3 项目完成、7/7 测试通过、总耗时约 16 分钟。
**返工分布是核心数据**：CLI 1 轮、领域建模 **0 轮**（首次 check/test 全过：
多字段构造器、断言、错误路径、嵌套 JSON 校验、聚合排序）、**Web 14 轮**。

**判决**（Codex 原话）：「语言简报强到 CLI 与领域工作几乎立即高产，但对 Web
项目引导/测试尚不自含；测试运行器绿色结果与未处理运行时错误并存是最尖锐的
信任问题。」
...
| ID | 发现 | 处置 |
|---|---|---|
| **BLD-D1（最尖锐信任问题）** | `velar test` 打印未处理的 `document.createElement` 错误、仍退出 0 且报 "1 passed, 0 failed" —— **盲测独立复现了 ASY-D2 家族**，且路径可能不同（headless 测试导入了浏览器入口的模块级 DOM 错误 vs detached vs flush） | **N-2b-2 的 ASY-D2 升级为统一立场**：测试期间**任何**无主错误（detached/flush/DOM/模块 init）都判该测试失败 —— 一条规则，不是按通道打地鼠。N-2w 已修 flush 通道；其余通道随 N-2b-2 |
| **BLD-U1（14 轮返工的根源）** | **Web 项目激活不在简报里**：velar.json、formatVersion、`@velarscript/web` 扩展身份、最小 manifest、`test_*` 发现规则、挂载入口与纯可测模块的分离、dist/index.html 归属 —— 全靠猜。诊断教了 formatVersion **却不教扩展包名** | 三路：① 简报加「项目启动」一节（core+web 最小 manifest 各一份，fence 门禁编译）→ N-3 + 镜像规则；② 配置诊断给出**完整合法 web 配置示例** → N-2b-2；③ 核查 `velar create` 是否本就该脚手架掉这一切、简报改教 create → 波 S 验收时一并看 |
| **BLD-P1（正面凭证）** | 数据建模优先、类、错误路径、集合链、Type.parse 边界校验 —— **零返工首过**；Web 激活后语言表面（state/computed/action、事件、键控、深变更）真实浏览器行为正确 | 记档：**语言核心与简报过关，瓶颈是引导不是语言** —— 盲测 KPI 的第一个真实基线（16 分钟 / 7 测试 / 3 构建） |
...
## 裁判迁移（2026-08-13，Codex 执行：VelarOS-Lite → db629df）

**结果**：可比通过率 **33/100 → 100/100**；四项目 check 全绿、82 Core +
54 浏览器矩阵 + 服务器与安装包验收全过；**没有任何一处卡住超过两轮**。
Desktop 15.6MB / 20MB 预算。诊断教学质量：**按家族 5/9 一步教会；按条数
8/24**（十条重复的枚举/wire 消息拖低了条数分）。

### 教学缺口工单（每条「否」都是语言的工单）

| ID | 发现 | 处置 |
|---|---|---|
| **MIG-1（本轮最强发现）** | **D42 枚举/字符串诊断的第一推荐对开放 wire 协议是行为破坏的**：消息教 `Kind.parse(text) == Kind.member` 优先 —— 但 `parse` 对未知值**抛出**，前向兼容的协议处理器必须忽略未知事件；照做后 41/42（未知未来事件测试崩）。第二形态 `str(member) == text` 才是该场景的对（Codex 二轮收敛）。**「编译干净但运行时破坏」是最坏的诊断建议类** | 两半：**(i)** 消息措辞修——明说 parse 抛出、开放协议该用 str 形态（我在合流时顺手改）；**(ii)** **FLW-N3 升格**：让 `Kind.is(raw)` 建立收窄（charter §6 本就指引作者用它；落地后开放协议模式变成一等拼写 `if Kind.is(raw): match raw`）→ 归批次 K，可否决 |
| **MIG-2** | **未类型化的导出 computed 只在消费端报**（四条 unknown 消息，零条指向导出边界）：正确诊断属于 export 处，教 `export const name: () -> T = computed(...)` | 归批次 K：导出边界诊断 |
| **MIG-3** | **Desktop 体积预算失败只报总量**不报构成（新的强制工具链 ~13.7MB 需要上游考古才敢改预算） | 归批次 L：打包失败带分项体积 |
| **MIG-4** | `invert` 的移除没有定向引导（撞到的是无关的语句边界消息）——移除拼写应有 VEL2033 式引导，invert 漏了 | 合流时顺手补 |

### 一步教会的正面凭证

存在性检查（`!= null` 精确拼写 + 真值/存在之辨）、严格 pop 的 `?? ""` 噪音
定位、VEL4027 双选项带逐字语法、死检查识别（「恒假——删检查或声明 T?」）、
恒非空工厂比较——全部一步收敛。

### D48 侧注

Lite 无 velar/javascript / text-buffer 导入、无独立编辑器项目 —— 编辑器侧
迁移无对象，D48 的包通道验证由本仓验收门禁承担（已在波 S 落地）。


---

## 运维根因：desktop-worker 挂起（2026-08-13 关闭）

四次发作、每次损失 30-50 分钟的门禁冻结，根因**不是**慢，是**永不到达**：

`fs.watch(path, {recursive: true})` 在 darwin 上**异步武装** FSEvents 流，流从
「武装完成那一刻」起算 —— 在武装前写入的变更**根本不会被报告**（不是迟到，是
永不）。而这个 promise 之上**没有任何一层有期限**：worker 的 `nextFileWatch`
按设计是无界拉取、测试夹具对每次调用零期限、`node --test` 默认
`--test-timeout=0`。两个进程 0% CPU 永久停泊 —— 与记录的形态完全一致。

**根因在产品代码里留了指纹**：`packages/desktop/src/compiler.ts:882` 给
`watchNext` 传 timeout **0**，而给其他每一个能力调用传 **30000**。
唯一被产品有意留成无界的那个等待，正是冻结套件的那个。

**复现是测量出来的不是猜的**：低负载 60 次尝试 0 丢失；高负载 40 次 **丢 4**，
其中一次的窗口长达 **25 秒**（证明是永久丢失）；武装与写入之间加 400ms 宽限
→ 更高负载下 40 次 0 丢失（隔离出成因）。

**修复**：全部等待加期限（消息点名操作/worker pid/可能成因）、丢失通知
250ms 重触发上限 30s、host 内三处真无界等待（终端所有权握手、两处子进程
stdin 帧写入）加期限、失败的终端 spawn 不再让败者 rejection 升级成 host 退出、
`before` 钩子回收被杀运行的残留（双重年龄下限保并发套件安全）、
`npm test` 加 `--test-timeout=120000` 系统性兜底。

### 待用户裁决（产品语义，实施者不擅改）

**WATCH-1**：产品的 `watchFiles`/`watchNext` 无界，且 macOS 会静默丢弃与
watch 启动并发的变更 —— **Desktop 应用若在 `watchFiles` 之后立刻写入，可能
