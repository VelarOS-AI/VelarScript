...
5. **`computed(…)` 函数形态保留**（见下方「最终形态」一节）。
   改的只有一处：`const x = computed(…)` / `let x = computed(…)` **被拒绝**，
   诊断教「写 `computed x = E`」（VEL5055 反向）。**不禁嵌套** ——
   `const bag = {read: computed(…)}` 合法。
...
computed(() => E)                 // 保留：一个能传的、带缓存的读取器
const x = computed(…)             // 拒绝：要声明就用 computed x = E
```

**语言只规定「声明只有一种写法」；那个函数是工具，不是规定。**

### 为什么保留函数形态（我一度主张退役，被推翻，理由记下来）

我曾主张退役，论据是：三处能力（记录字段、工厂返回、条件表达式）
**用普通 lambda 全都能做**（实测确认），所以函数唯一不可替代的贡献是**缓存**；
而缓存是优化，按项目的优化分层原则「可选 API 只在有真实证据表明自动的覆盖不了时
才加」，且仓内**零处**使用表达式形态 —— 故应退役。

**用户推翻，理由成立**：优化分层规则管的是**「要不要加」一个可选 API**，
不是**「要不要拆掉」一个已经存在、仍然可用、拆除本身有代价的东西**。
我把规则用反了方向。

**且「让用户自己决定要不要用」本身是对的**：语言的职责是保证
**声明派生值只有一种写法**；至于要不要为一个传递出去的读取器买缓存，
那是作者对自己代码的判断，不是语言该替他做的决定。

### 三处能力与它们的正路

| 你要的 | 写法 |
|---|---|
| 给派生值起名字 | `computed x = E` |
| 传一个活读取器（不要缓存） | `() => E` —— 普通 lambda，实测三处全通 |
| 传一个活读取器（**要**缓存） | `computed(() => E)` |
