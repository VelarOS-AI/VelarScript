...
## 第 140 条 —— 标准模块的运行时不得导出接口未声明的名字（门禁）

### 起因：第 137 条暴露的不是缺陷，是**退役程序的漏洞**

波 J2 退役 `Blob` 时发现：**删接口不够**。逃生通道直接够得到运行时模块 ——
`import js unsafe {readBlob} from "velar/fs"` 绕过接口打到运行时函数本身，
必须连运行时一起删。这一条是它手工注意到的；**下一次不一定有人注意到。**

### 追查这一族的结果：**类是空的**（如实记录负结果）

我按此追查「运行时有而接口没有」的普遍情况，结论是**当前不存在**：

| 探针 | 结果 |
|---|---|
| 36 个标准模块表面，运行时导出 vs 接口声明 | **零盈余** |
| `import js unsafe {__velarNodeHostInvoke}` | VEL3007 保留前缀，编译期拒绝 |
| `import js unsafe {totallyMadeUp} from "velar/fs"` | `velar check` **通过**（`unsafe` 的定义就是作者自负），`velar build` 通过 |
| 上一条 `velar run` | `SyntaxError: does not provide an export named 'totallyMadeUp'` —— **模块加载期响亮失败** |

即：内部名由 `__velar` 前缀规则挡住；不存在的名字在加载期立刻炸而不是静默走错；
**没有洞。** 但「零盈余」这件事**没有任何东西在维持**。

### 裁决：把这条不变式变成门禁

`check:boundaries` 增加一项：逐个标准模块比对运行时导出与接口声明，
盈余即失败。`__velar` 前缀按规则豁免（它自有 VEL3007 保护），**不是按名单豁免**。

这条同时把**退役完整性**钉死了：将来任何人只删接口不删运行时，门禁会红。

### 实施记录：本条的第一版自己就犯了本文命名的那个毛病

第一版把三个扩展合并成一张表再审计。而 `new Map([...core, ...combined])`
**后来者覆盖先前者** —— Desktop 自带一份 `velar/fs`，于是它盖掉了 Node 那份，
**审计只查了两份实现中的一份，却报告说查完了。**

注入实测：往 Node 的 `velar/fs` 运行时塞一个未声明导出，合并版**绿灯放过**。
改成逐扩展审计后同一注入被抓到，且审计表面数从 **30 升到 36** ——
那 6 个正是被覆盖掉的。

**这正是第 134 条的族**（一份本该逐项派生的清单被一次合并悄悄压扁），
也正是「门禁覆盖面低于它自称的覆盖面」那一类。记在这里，
因为**给这个族命名的人下一小时就又犯了一次**，说明它不是粗心，是这个形状真的很滑。
