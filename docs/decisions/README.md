...
| [D32](D32-COERCION-AND-OWNERSHIP.md) | 第四轮语法排查：强转洞、浮动 Promise、混用括号 | Coercion holes close (f-string and `str()` get a conversion allowlist) and fire-and-forget gets the `detach` statement instead of a wrapper function. |
