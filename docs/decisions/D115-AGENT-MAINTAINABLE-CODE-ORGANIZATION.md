...
| **P7** | D34 路线图车道择一启动（姊妹仓迁移**不做**——所有者 2026-09-05 裁定，不在本仓工作范围） | 所有者另行下令 |
...
## 修订（2026-09-06，R1f 落地后）

- §三 `analysis/analyzer.ts（门面 … ≤800 行）` 与 §五 P2 的完成条件「`analyzer.ts` ≤ 800 行」**修订**：
  `analyzer.ts` 是**组合根**——只含协作者实时读取的状态字段、protected 缝、宿主构造器与分派器，
  **不含分析逻辑**；行数上限不适用于它，代之以 `compiler-architecture.md` 里的分段预算
  （字段 / 构造器 / 缝 / 分派器各一行）。理由与实证见 D114「R1f 落地」：宿主构造器读 `private`
  状态，搬出类外即 TS2341，唯一出路是放宽公开面，那比一个 3,251 行的组合根更伤维护。
- P2 其余条件成立：compiler 下除组合根外无 >800 行源文件（`lexer.ts` 714、`index.ts` 743），
  允许名单 37 文件 / 41 函数，全在 tests/ 与 web / node / cli 包——那是 P4 / P5 的对象。
...
十一波（R3-0、R3a–R3e、R4a–R4d、R6）当天落地，每波发射产物逐字节不变、门禁绿、allowlist 只降：28 文件 / 34 函数 → 15 文件 /
