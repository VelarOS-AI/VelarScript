...
**使命定位裁决（2026-08-12，用户口述）→ D33 + docs/why-velarscript.md**：
核心宗旨（用户原话）：**「AI 写和改，人来读」** —— 解决 AI 时代模型代码不可
维护的问题；写者与维护者都是 AI（凭 JS/Python 先验零样本上手，下一个改代码的
是没见过这文件的另一个 AI 会话），人只读、只确认意图；编译器对每次修改兜底。
最佳实践典章 docs/best-practices.md 已落地（全部代码围栏过 check:docs 编译，
教错拼写=门禁红），AI Skill（D33 第32条）以其为正典源。

**优势与高级功能路线图 → D34（雏形，活文档）**：语言框架三位一体的优势车道
存档 —— 两条从落笔生效的法律（**扩展加词汇不加同义词**、`velar skill` 聚合
扩展章节）；已成事实待包装四项（属性级响应 0/0 证据、类型即验证器、Look 静态
提取、todo 实测 **16 KB gzip**）；性能车道 P1-P5（体积门禁化、自动分包、完美
死代码消除、检查消除「安全不减速」、公开基准）；高级功能 A1-A5（时间旅行
devtools「运行时也可读」最高推荐、端到端类型服务端函数、能力安全 manifest、
确定性测试、领域扩展维持等证据）；**WASM/多线程留门**（经扩展协议、不污染
Core、今天不承诺）。全部排三阶段之后；每条启动前需单独规格。

**第五轮排查（类型/语义深水区）+ 两项用户裁决 → D35**：九连阳确认内核扎实
（闭包按迭代捕获、split 码点、**函数参数逆变健全**、递归类型、override 强制、
abstract 拒实例化、JSON 编译期白名单、联合穷尽 match 免 else）；五轮总结论 ——
毒瘤不在类型内核，聚集在 JS 边界渗漏（批次 H 正在拔）。用户裁决两项：
①并行异步必须有好拼写 → **`all(记录)` → `Promise<记录>`**（逐字段类型原生、
结果带名、零元组零新语法；`race` 放宽为联合结果）；②纯计算标准库常驻命名
空间 → 法律「**能算的不用导，能碰外界的必须导**」，**大写开头且零发明**（用户两次裁决：大写开头 + 指出 Async/Text 与 JS 内建的
混淆风险 + 「不能为了兼容破坏 vel 自身的统一」）：最终 roster 三个 ——
`Json.`（源自 JS 全局、拼写归一 PascalCase，`JSON.` 给定向指引；**Vel 自身
统一 > 亲代兼容**，`===`→`==` 同款先例，曾短暂定全大写被用户推翻）、
`Promise.`（组合子挂既有类型，`Promise.all` 正中 JS 肌肉记忆）、`Look.`
（挂既有类型）；**`Text.` 不设立**，velar/text 存留四
函数方法化归 string（L1 走完、模块退役），消除「方法还是命名空间」二选一；
法律升级「常驻命名空间不发明名字」；配 JS 全局指引族（Object.keys/Array.from/
Number.isInteger/Math./String()）；能力模块 import 行即审计信号，支撑 D34-A3；
`range` 进 prelude 灭盲测头号 D 项。批次 K，排 A 后。第五轮遗留四项已全部裁决落规格：字段
初始化器 self 消息 + for live 语义成文 → D31 第 28 条（扩为五小项）；
`removeLast()` 严格弹尾 → D29 附议 D；字符列表**改判不加 `chars()`** —— 实测
`split("")` 已按码点切、拼写已存在，缺的是成文（charter split 行未写空分隔符
行为），加 `chars()` 即同义词、违反 D34「不加同义词」法 → D29 附议 C
（成文 + stdlib 95 行分块层迁移）。

**第六轮排查（源码安全 + 亲代分叉 + 链语义）→ D36（已批准）**：可选链恢复
JS 整尾短路（`b.inner?.deep.upper()` 现被拒、强迫给非可选字段写 `?.` 是语义
谎言；修后「`?.` 位置 = 真实可空位置」，归 F）；原生属性已知表检查（`clas`
现静默通过；已知表 ∪ data-*/aria-* ∪ 指令族，未知名带最近拼写建议，归 I）；
字符串外 bidi 控制字符报错（Trojan Source：注释内 RLO 现静默通过，「人来读」
的语言把审读欺骗当命门，归 E）；**新增 `/* */` 块注释**（用户推翻「只给指引」：
可嵌套（Rust 先例）+ 行纪律（行内同行闭合、跨行独占整行，不成为第二续行机制），
归 E）；换位/链式赋值定向指引（归 F）。**父母法律立法**（用户原话：
「本质上是运行在 js 上的，所以行为优先 js，只不过写法更像 py —— 父亲和母亲
的区别」；角色经用户复核：**JS 是母亲** —— 母体承载运行、行为随母，
**Python 是父亲** —— 姓随父、拼写名分随父）：语义问母亲、拼写随父亲、母亲的
行为是陷阱时才移除；负取模 -1 成文。
**第 41 条 NaN 统一（用户裁定缺陷并批准）**：`==`/`!=` 采 SameValueZero
（`NaN == NaN` 真，自反性恢复，与 Set/Map/List 成员操作合一；类型驱动消除保
零税）；`isNaN()`/`isFinite()` 补齐谓词三件套；`sum/min/max/sorted` NaN 围栏
抛错；不做受检算术、`<` 保持 IEEE（防复议记档）。归批次 A（增至十项）。

**第七轮排查（Look 深处 + Web 错误质量）→ D37（已批准）**：Look 字符串值按
属性关键字表收紧（现状 `padding = "big"` 任意垃圾通过、`"12px"` 是单位第二
拼写，归 I；**42-补**：用户实地标本 `gridTemplateColumns = "240px minmax(0,
1fr)"` 实测放行 —— D11 机制优质但 grid-template 族漏注册，修法 = 补注册 +
全量注册审计 + 逐属性回归固化完备性）；事件名已知表（`on:clik` 现静默，归 I）；组件声明强制 PascalCase
（`component card:` 现通过、JSX 使用位被当原生标签，归 I）；**跨扩展指引族**
（Core 项目写 JSX/component/state 现是裸死胡同 → 教 velar.json 加 web，归 E）；
void 元素带子节点与单位混算的消息定向（归 I/E）。守卫正面清单已记档（JSX
闭合/bind 目标/重复属性/Look 未知与重复属性等均扎实）。同轮裁决：`async`
语句关键字**维持不换 `sync`**（sync 字面义与行为相反 —— 反向误解比生僻更危险；
Python 有语句位独立 async 先例）；组件内异步五选一决策表成文（resource/action/
mounted-await/watch-sync/async 旁路）；**mounted 卸载竞态 = 安全丢弃**（用户
裁决：实例自有单元写入打墓碑 no-op；模块 state 照常生效；rejection 仍上报 ——
丢写入不丢失败可见性）。

**第九轮（生成性评审：可加语法与可优化处）→ D39（全部批准）**：
`using` 资源绑定（JS 母亲正在原生加 using；Lite W-118/120 证据；close() 契约、
LIFO 释放、失败不吞不盖，归新批次 M）；`try` 表达式（`const u = try
User.parse(x)` → `User?`，Swift try? 先例，Lite 手写 tryParse 证据；结果必须
被消费，裸语句拒绝，归 M）；时长进时间 API（`sleep(2s)`，ms/s 与 Duration
提升 Core，裸 number 退役指引，归 K）；`test "名字":` 块（测试即 owner 读的
规格，def test_* 退役，归 M）；格式化器 JSX 换行政策（403 字符行终结，归 L）；
stdlib 错误码约定（消息归人、code 归程序，杀 Lite 前缀匹配，归 K）；明确不加
七项记档（管道/切片/Set 字面量/Context——模块 state 即答案/decimal/装饰器/
for 过滤）。全局序：J ✓ → A（在途）→ K → E/F → L → I+B → G → M → C/D。

**`%` 与单位系统三层裁决 → D40**：① `%` 提升 Core 的「标量徽章模型」初版
裁决被**用户挑战撤销**（「外部定义 CSS 值不会混乱冲突吗」→ 暴露同一表达式
`half * 2` 换位置换结果 —— 本次评审追杀的陷阱类别命中评审者本人，全程记档）。
终版：`%` 留 web（一套 CSS 规则、位置无关、四道防线已在）；Node 业务百分数
用 0.15（Lite 零证据）；混算守卫 web 已有；D39 的 ms/s/Duration 提升不受
影响；教训成文「双语境语义提案必须先过换位置测试」。② 单位系统公开化 =
升格 web 已实战五个月的扩展钩子为公开契约，**首个非 web 消费者出现时落地**
（D34-A5 同门控）。③ 源码级 `unit km` 声明**不做**（上下文敏感词法 + 量纲
代数即类型级编程，charter 规则 4 排除）。④ 第 60 条：单位值在普通代码的
边界实测**全绿无静默问题**（== 按值、算术照常、记录/验证认、< 有原则拒绝、
sorted 逼显式比较器、f-string/JSON 正确拦），三抛光：VEL4026 对扩展值只教
print（stringify 出口失准，扩展文本钩子获第一份证据）、比较拒绝消息补原因、
边界矩阵进 charter §17 与简报。

**待用户裁决队列（马拉松协议第 1 条产物）→ 已于 2026-08-12 全部裁决，见 D41**：
① 泛型内文本转换被拒（`def label<T>(v: T): f"{v}"` 不可写）→ 用户裁「类型级
编程做可控、规则简单支持优先」→ **封闭词汇有界多态 `<T: Text>`**（约束名编译器
自有不可用户定义、仅 `def` 可带、只做调用点检查+体内解锁；与 charter 规则 4
排除的 TS 式类型级编程属不同品类；未来 Comparable/Key/Data 需站点计数门控）。
② **用户指出 `removeLast` 与 `pop` 重复**（编排代理引入的规则 3 违例）→
`pop` 严格化（空表/越界抛，与 `[]` 及父亲 Python 一致）、**删 `removeLast`**、
排空惯用法改大小守卫、`?? fallback` 噪音退役。③ `str()` 非逃生阀成文、
④ 扩展文本钩子（单位值可进 f-string，方向已定）、⑤ `velar/math` 的
isFinite/isInteger 函数删除（双拼写）—— 均按推荐执行。
**α-9 判断留痕**（不需裁决但记档）：模块环规范顺序选「项目主 entry 为第一根」
而非最保守规则，后者会拒掉测试套件钉住的三个合法程序；两驱动器已一致。

**相等与有序的统一收紧 → D42（用户裁决「number 不能和 string 做比较」后批准）**：
实测发现 **`==` 对操作数类型零检查**（`1 == "1"`、`user == "a"`、`true == 1`
全部编译通过，恒为假；只有 `<` 在查类型）—— 去掉 JS 强转换来的是「静默逻辑
bug」，净收益为零。**第 64 条**：`==`/`!=` 要求两操作数类型有交集（按可赋性
判定），无交集即拒；完整情形表含两处防误伤正面断言（`T? == null` 必须保留、
else 分支收窄为联合不误伤）。**第 65 条**：枚举实测**标称身份在运行时不存在**
（`A.x == B.x` 为 true、跨枚举可排序、枚举与裸字符串可混排、`[high,low,normal]
.sorted()` 静默出字母序）→ **枚举不进 `Comparable`**，排序路径收紧到与直接 `<`
一致；三条排序谓词不一致（ORD-1/2/3）一并关闭，四套机制合并为一个判定点
（ORD-2 是三者中唯一正确的，维持拒绝）。**显式例外记档**：枚举 vs `string`
相等必须拒绝 —— 枚举→string 的可赋性是**单向出口**（发线上值），相等是对称的，
允许它等于开一条绕过 `Enum.parse` 的读入路径。归批次 A′，是批次 M 实现
`Comparable` 的前置。

**设计方法论纠正（2026-08-12 用户，最高约束，优先于既有的证据纪律）**：

> 原话：「不要等什么东西都到用到了才加，这不是一个语言设计者应该有的思维
> 方式，而是尽可能考虑到所有情况，在边界设计之内」

判据一句：**证据可以门控「这个品类该不该存在」，不能门控「已划边界内的
完整性」**。边界一旦划定，必须穷尽设计完，不得只发一个成员、等用户撞墙再补。

**对本项目尤其成立**：写代码的是 AI。AI 撞到缺口**不会报 bug，它会写
workaround，然后 workaround 变成代码库的形状** —— 等证据等于等一个永不到来
的信号，同时语言的缺席已经在悄悄扭曲它本该塑形的代码。

**完整性要用系统方法导出，不靠想象**：如「哪些约束是必要的」= 反查编译器中
**所有拒绝无约束类型参数的位置**，该集合即完整词汇；「排除某候选并写明理由」
（Vel 只有一种数值类型，故 `Numeric` 约束无意义）与「纳入某候选」同等重要。
封闭集合（用户永不可定义）正是让完整性负担得起的前提。

**已按此重判的门控**：
- 类型约束 ≥5 站点（D41 第 61 条）→ **撤销**，完整词汇由编译器反查导出。
- 单位系统公开化「等首个非 web 消费者」（D40 第 58 条）→ **撤销**，机制已存在
  且 web 实战五个月，公开契约现在设计。
- 枚举穷尽表 ≥5 站点（D29 第 8b）→ **拆开**：表本身是糖可缓；但枚举缺
  「遍历全部成员」（`Status.values()`）是真完整性缺口，现在补。
- `Int` 精化类型 ≥8 站点（D29 附议 A）→ **门控保留**（精化类型是新品类，
  不是补完）。
- `velar/game` 等领域扩展（D34-A5）→ **门控保留**（新战线，不是补完）。

**待办**：对每个既有特性面做一轮系统完整性审计（用与约束词汇相同的反查方法），
产出各面的缺口清单。排在修复波 2 之后。

**类表面五条收尾（2026-08-12 用户「你决策」委托 → D45 定案）**：
① 类名不是一等值（第 75 条：直接调用/静态访问/extends/类型位/模式/导出六种
位置合法，别名与传递拒绝，工厂写 `() => P()`；与抽象类既有行为拉齐）。
② override 签名严格不变成文（第 76 条）。③ 类家族 match 必须可证穷尽
（第 77 条：不可反驳收尾或联合全覆盖，对齐枚举）。④ extern 类不可继承列为
有意缺席 + 定向诊断（第 78 条）。⑤ setter 有意缺席 + 定向诊断（第 79 条）。
第 75/77 并入 N-2，第 76/78/79 并入 N-3。用户同日指示「按顺序继续推进，
有问题停下来问我」。

**反引号字符串（2026-08-13 用户裁决 → D46 第 80 条）**：`` `...` `` 与
`"..."` 语义完全相同、串内 `"` 免转义 —— 动机是 JSON-in-string 夹具的转义痛点
（用户原例 `parseJson("{\"name\":\"Nova\"...")`）。边界一次设计完：前缀正交
（f/r/rf × 两种引号全组合）、`${` 是字面文本（不做 JS 模板插值，AI 简报一行
治理）、裸换行仍教布局串、单引号仍拒。典章规则「写时随手、读时统一」：velar
format 按确定性规则归一（默认 `"`；含 `"` 用反引号；都含取转义少者）—— Black
同构先例，规则 3 不破。归批次 E/F 词法波。

**晨间五裁决（2026-08-13 用户整体批准 → D47）**：① `equals(a, b)` 深度相等
进 prelude（函数不是方法 —— 记录无方法且会撞同名字段；交集规则适用；类拒绝；
环报错）。② `\u{...}` 转义 + **双向控制字符源文件全文禁裸**（字符串与注释都是
木马源载体；ZWJ/变体选择符明确不禁 —— emoji 构成字符）。③ extern 实参只读
成文。④ **bind 开成员路径**（`bind:value={form.name}`）+ radio/checkbox 组的
`bind:group` 拼写（Svelte 同款）+ 事件对象无 target 成文 + 删除永不可触发的
死教学。⑤ unknown 是静态承诺边界成文（RDO-1 关闭）。
归属：①④ → N-2b；② → E/F；③⑤ → N-3。

**标准库成员边界（2026-08-13 用户裁决 → D48）**：编辑器项目把
velar/javascript（36KB JS 分析）与 velar/text-buffer（22KB）带进 stdlib ——
**应用需求反向绑架语言表面的实例，判违规**。第 86 条：`velar/*` 是语言拥有的
封闭词汇，成员仅限**普适计算**或**能力原语**；域功能永为可安装库（TS/npm
心智，机制已存在且经审计验证）。第 87 条：两模块迁出为包、CLI 语言服务改
...
**终局裁决授权（2026-08-14 用户，覆盖此前的 pending 限制）→ D50**：
编排代理获授权自行裁决剩余全部开放设计问题。已定案：**第 89 条** 错误判别
只有一套分类 —— **类**；`code` 是类名的字符串投影（恒等映射，回答了 D39-55
六个缺口全部；不引入平行 code roster —— 那是规则 3 违例）；stdlib 错误类家族
按「调用方会写出不同恢复逻辑」反查抛错点系统导出，每类须有真实抛错点。
**第 90 条** 纯计算全部常驻：Json 补全全量、**建立 `Text.`**（推翻 D35 的
「改用方法」—— 方法化会让核心字符串成员表翻倍）、`Json.deepEqual` 退役由
`equals` 唯一承担（必要时扩展 equals 吸收其能力）。**第 91 条** 文件监视武装
语义成文不加魔法。**第 92 条** 死表面清理（`Opacity` 不可达类型名等）。
**第 93 条** 剩余批次顺序：R2 → G → M → L → C/D，各自分支、串行合并。

**马拉松工作协议（2026-08-12 用户授权，最高约束，优先于本节其余排期）**：

1. **设计层改动必须先与用户探讨** —— 语言语义/拼写/表面的任何变更，即使发现
   得再确凿，也只记录不擅动；用户不在时**记录后继续处理别处**，回来再汇报。
2. **实现层由代理自行决策** —— 编译器内部结构、发射策略、诊断实现、测试组织、
   批次编排全部自主，不必等待。
3. **注意性能** —— 新增检查（SameValueZero 等式、丢弃结果分析、模块环分析等）
   不得引入可测量的编译期或运行期回归；有疑即量，量了记档。
4. **持续挖实现层漏洞并修掉**（阶段②提前并入马拉松）：对抗搜捕不再等语言冻结，
   与批次队列并行推进；发现物按「实现缺陷 → 直接修；语义设计问题 → 记录待议」
   二分。
5. **不因用户离线而停** —— 队列走完一批推一批，汇报累积到用户回来一次性给出。

**马拉松发现 #1（实现层，代理自主处理）**：`tests/performance.test.ts` 只有
**编译期**预算（初次 5s / 增量 2s / 搜索与语言服务器若干），**没有运行期基准**
—— 而批次 A 的 SameValueZero 等式降级、丢弃结果分析、附议 B 索引收紧全部影响
**产物运行速度**。缺口即「性能回归无门禁」。处理：批次 A 落地后新增运行期
基准套件（等式热循环、集合操作、响应式更新、渲染），先量基线再记预算；
D34-P5 的 js-framework-benchmark 是其对外形态，本项是内部门禁形态。

**阶段路线图（2026-08-12 用户定）**：① 现阶段继续评审语法（四轮已毕，批次
H-J 在途/排队）→ ② 语言确认没问题后重跑对抗搜捕找漏洞（新增笔误语料、异步
笔误、强转垃圾输出等维度）→ ③ 第五期盲测升级为 **skill 闭环**：AI 只带
内置 skill 盲写真实程序，观察、记账、修语言或修 skill，循环至收敛 —— 测的
是「skill + 诊断」这套完整分发面，即产品本身。
既有设计法则升格为使命推论（一个明显拼写 = AI 产出可维护；诊断即教学 = AI
一轮自愈；盲测是核心 KPI）。随之立项：AI Skill 随安装包分发（`velar skill` +
create-velar 生成 AGENTS.md + 防漂移门禁 + 每批次同步维护规则）；JS 逃生通道
产品化（escape-hatches.md 决策树 + 可读 JS 弹射路径门禁化 + W-23/W-25 挂账）；
兼容性政策成文（**永不承诺向后兼容**，教学式迁移；短生命周期产品锁版本，
稳定通道由外部证据触发）。批次 J 排在 H 之后、A 之前。README 顶部与 charter
§1 前言的定位改写排 J（避开 H 的在途 charter/README 编辑）。
