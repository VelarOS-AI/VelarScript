...
## 6. L4 实施裁决追记（2026-08-27）

1. **§2 表修正（strict/plain 写反了）**：zod 4 的普通 `z.object` 剥离
   多余键——与 Vel `parse` 的行为**精确等价**；`z.strictObject` 拒绝多余
   键——Vel 表达不了。修正为：`z.object` 映射为精确等价；`z.strictObject`
   映射为记录 + **计数上报的放宽**（实现现状即此形态，予以追认）。
   封闭记录是否进语言 = 待所有者裁决的语言设计题（证据：真目标 122 个
   strict 对象全部放宽）。
2. **§4 双条件放宽为枚举放宽类**：`zod.parse 通过 ⟺ Type.parse 通过,
   除既定放宽类外`，每类必须有样本锁定，且每个声明的分歧必须是放宽、
   永不收窄。追认实现的六类清单。
3. **§1 的旗舰目标改派**：`ChatStreamEvent` 在 platform 是纯 TS 类型,
   无 zod 对应，指定机制够不着。裁决：**platform 侧为它补 zod schema**
   （壳边界本就需要运行时校验的单一权威），列入 P1 产品侧任务；L4 不加
   第二套机制。
4. 真目标暴露的语言缺口三条：数值字面量单例（21 处 protocolVersion 钉
   定,现全部降级 unknown）、封闭记录（见第 1 条）、optional/nullable
   不对称（§2 已裁归一化,追认为放宽类记录）。前两条为**语言设计题,
   已按协议向所有者汇报待裁**——推荐方向见汇报,未裁前生成器的豁免
   通道即现行答案。
