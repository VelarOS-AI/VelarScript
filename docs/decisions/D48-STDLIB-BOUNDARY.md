# D48 — 标准库成员边界 + 库分发模型（用户裁决 2026-08-13，已实施）

> “编辑器域功能不进入 Standard”仍然有效；“迁入本仓 `libraries/` 并由本仓发布”
> 已由 D88 废止。当前应用实现归实际消费项目所有。
...
1. **两模块迁出为可安装包**，源码归入 `libraries/`，不与 `packages/`
   下的工具链实现同层，也不进入工具链版本锁和发布候选。包机制**已存在且已验证**
   （`velar.entry`，审计七/八：可导入、多版本守卫、适配器模式全通）。
