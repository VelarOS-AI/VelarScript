...
非标准库、外部适配器与编辑器接入套件。它是一个包含多个独立 npm 包的
monorepo，不是一个聚合依赖包，也不是 VelarScript Core 的第二个工作区层。
...
3. `VelarScript-Libraries` 中的 `@velarscript/*` 是显式安装、独立版本的官方
   可选包；
...
- 仓库使用 `packages/*`，一个目录对应一个 npm 包；不得建立
  `@velarscript/library` 聚合巨包。
- 每包拥有自己的 SemVer、CHANGELOG、Git 标签和发布候选；不与工具链版本同步。
- VelarScript 源包使用现有 `velar.entry`、`velar.targets` 与
  `velar.requires.capabilities`；编辑器或部署工具使用普通 npm `exports`。
...
- 发布必须从打包后的 tarball 验证：内容清单、许可证、完整性、隔离安装、支持
  目标执行和最老/最新支持工具链兼容矩阵。发布与 Git 推送仍是单独授权边界。
- 包状态明确标为 `experimental`、`stable` 或 `deprecated`。已发布版本不
  unpublish；退役使用 npm deprecation 与迁移说明。
...
伴生仓库可以提供 `@velarscript/editor-kit`，集中维护语言 id、`.vel` 文件关联、
...
首批低争议候选为 `@velarscript/text-buffer`、`noise`、`msgpack`、
...
- D48 的“编辑器域功能不进入 Standard、通过 npm 安装”继续有效；其“放在
  Core 的 `libraries/`”仍由 D88 废止。
