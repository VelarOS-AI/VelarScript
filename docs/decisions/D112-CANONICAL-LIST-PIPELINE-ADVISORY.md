...
List 声明 + 单槽 `for`”只是在把一个 List 做逐项投影、扁平投影，并可选地先用
...
1. 目标是紧邻循环声明的空 `List`，来源是普通 List 绑定或由静态记录数据字段
   构成的稳定路径；
2. 循环同步、单槽，循环体只有一次写入目标 List 的 `append` 或 `extend`；
...
普通函数调用、class getter、索引读取、await、计算来源、双槽循环、第二条循环体
语句、目标状态依赖及任意其他副作用都保持沉默。
