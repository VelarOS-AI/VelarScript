...
export const VELAR_COLLECTION_LOWERING_RUNTIME = [
        ...
        "function* __velarReactiveMapKeyIterator(value) { const size = __velarCollectionSetMapMapSize(value); if (size > __velarMaxCollectionItems) throw new __velarCollectionSetMapNativeRangeError(\"A Map cannot exceed 1000000 entries\"); __velarReactiveCollectionTrack(value); const iterator = __velarCollectionSetMapMapKeys(value); while (true) { const step = __velarCollectionSetMapMapNext(iterator); if (step.done) return; yield __velarReactiveCollectionRead(value, __velarReactiveIterateKey, step.value); } }",
        "function* __velarReactiveRecordIterator(value) { const fields = __velarRecordFields(value, \"Record iteration\"); __velarReactiveCollectionTrack(value); for (let index = 0; index < fields.length; index += 1) yield __velarReactiveCollectionRead(value, __velarReactiveIterateKey, fields[index]); }",
        ...
        "    if (!__velarCollectionListIsInteger(index) || index < 0 || index >= value.length) throw new __VelarIndexError(\"List index must be an in-range integer\");",
        ...
        "  if (__velarIsRecord(value)) { const fields = __velarRecordFields(value, \"Record.keys\"); const output = new __velarCollectionNativeArray(fields.length); for (let index = 0; index < fields.length; index += 1) output[index] = __velarReactiveCollectionRead(value, __velarReactiveIterateKey, fields[index]); return output; }",
        ...
        "  const output = new __velarCollectionNativeArray(size); const iterator = __velarCollectionSetMapMapKeys(value); let index = 0; while (true) { const step = __velarCollectionSetMapMapNext(iterator); if (step.done) return output; output[index++] = __velarReactiveCollectionRead(value, __velarReactiveIterateKey, step.value); }",
        "}",
