  ...
  private selectFieldAndNarrowingHelpers(selection: HelperSelection): void {
    ...
    if (this.needsPrimitiveHelpers) {
      if (this.sharedRuntimeModules) {
        ...
        if (!this.needsCollectionHelpers) helpers.push(VELAR_INDEX_ERROR_RUNTIME); // D114 CO-U4b: `char`'s guard raises the IndexError the List lowering otherwise carries.
        helpers.push(VELAR_TEXT_METHOD_RUNTIME, VELAR_NUMBER_METHOD_RUNTIME);
      }
    }
