...
// CO-I6: the same sentence the emitted host error channel prints, through the
// same frame policy. This writer used to hand `failure.stack` straight to the
// console, so a nested `Promise.timeout` — an ordinary spelling, and the one
// that reaches this path — printed three frames that are all Node-internal or
// language-runtime, ignored `velar run --stack`, and named a sandbox directory
// the run had already deleted. `hostErrorTrace` is that policy's one
// implementation; `velar/async` already imports from the module that holds it.
function reportAsyncLoser(failure) { try { const runtime = globalThis[__velarAsyncDetachedRegistryKey]; if (runtime && typeof runtime.report === "function") { runtime.report(failure, { phase: "detached", detail: "async combinator loser", unhandled: true }); return null; } if (typeof __velarAsyncConsoleError === "function") __velarAsyncApply(__velarAsyncConsoleError, __velarAsyncConsole, ["Detached task failed: " + (__velarIsError(failure) ? hostErrorTrace(failure, "A detached task failed") : String(failure))]); } catch {} return null; }
