...
- Server owns the explicit `server.configuration` manifest path, application assembly, and
...
- Configuration parsing must remain bounded, strict, and fail closed. Project
  templates may choose `application.yml`, but the runtime path always comes
  from `velar.json` and is never discovered by filename.
