/**
 * Completion inside JSX: the element being opened, and the attributes that
 * element accepts — a component's own props and control slots, or the native
 * Web and SVG rosters.
 *
 * D115 P4 R3d: the two context scanners and the rosters they select from are
 * one concern; the Look side of the same file never reads either.
 */
import type {
...
import { unique } from "./completions.ts";
...
/** What may be written at a caret inside a JSX element, tag or attribute. */
export function completeJsx(context: CompilerProjectEditorCompletionContext): CompilerProjectEditorCompletionResult | undefined {
