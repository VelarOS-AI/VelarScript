...
test("[D116-3] a documentation-only change runs check, and only the tests that read a document", async () => {
  ...
  for (const path of ["docs/contributing/gates.md", "README.md", "docs/decisions/D116-SCOPED-GATES.md"]) {
    ...
    assert.ok(documentation.suites.node.includes("tests/server-port-zero.test.ts"), path);
...
test("[D116-3] a fixture or example project's change maps to the surfaces its own manifest declares", async () => {
  ...
  assert.deepEqual(owners(ownership, "tests/web-error-paths.test.ts"), ["cli", "compiler", "core", "web"]);
});
...
test("[D116-3] the heavy tier is disjoint from every quick-tier plan", async () => {
  const heavy = JSON.parse(await readFile(join(repositoryRoot, HEAVY_FILE), "utf8")) as {
    files: Record<string, { seconds: number; reason: string }>;
  };
  const listed = Object.keys(heavy.files);
  assert.ok(listed.length > 0, `${HEAVY_FILE} lists nothing, so the heavy tier is empty`);

  const everything = await buildPlan({ all: true });
  for (const file of listed) {
    assert.equal(everything.suites.node.includes(file), false, `${file} is heavy and still ran in the quick tier`);
    const entry = heavy.files[file]!;
    assert.ok(entry.seconds > 0, `${file} has no measured duration`);
    assert.ok(entry.reason.length > 0, `${file} has no reason`);
  }
  for (const file of everything.deferred.node) {
    assert.ok(listed.includes(file), `${file} is deferred by no entry in ${HEAVY_FILE}`);
  }
});
