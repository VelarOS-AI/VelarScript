...
test("toolchain release identity accepts its exact tag when sibling package tags share HEAD", () => {
  assert.equal(sourceHasExpectedTag({ tag: null, tags: ["@velarscript/core@0.12.1", "v0.12.1"] }, "v0.12.1"), true);
  assert.equal(sourceHasExpectedTag({ tag: null, tags: ["@velarscript/core@0.12.1"] }, "v0.12.1"), false);
  assert.equal(sourceHasExpectedTag({ tag: "v0.12.1" }, "v0.12.1"), true);
});
...
test("publication rehearsal emits reproducible verified package identities without publishing", async () => {
  ...
  try {
    ...
    assert.equal(firstManifest.version, "0.12.1");
