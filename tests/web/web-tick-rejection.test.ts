...
test("[LC-C1] the Node host is unchanged: it traces the failure and still rejects the tick()", () => {
  const node = run(awaited, "node");
  assert.match(node.output, /tick rejected: watch blew up/u, node.output);
  assert.match(node.output, /still running/u, node.output);
