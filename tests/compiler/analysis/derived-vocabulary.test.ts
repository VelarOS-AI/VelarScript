...
import { executeModule as executeModuleShared } from "../../support/execute-module.ts";
...
function executeModule(code: string) {
  ...
  return executeModuleShared(linked, { timeout: 5_000 });
}
...
test("[D55-127.2] the corpus carries the spellings the format gate reads", async () => {
  ...
  const source = await readFile(new URL("../../corpus/core.vel", import.meta.url), "utf8");
