...
test("component headers cannot declare type parameters", () => {
  ...
  assert.match(result.diagnostics[0]?.message ?? "", /Component 'Card' cannot declare type parameters; only 'def' functions take '<T>'/u);
});
...
test("CLI creates explicit format-v2 projects and rejects legacy manifests without overwriting user files", async () => {
  ...
  assert.equal(createdPackage.dependencies["@velarscript/web"], "^0.10.0");
  assert.equal(createdPackage.devDependencies["@velarscript/cli"], "^0.10.0");
...
test("CLI help is command-specific and malformed top-level invocations fail cleanly", () => {
  ...
  assert.equal(creatorVersion.stdout, "create-velar 0.10.0\n");
...
test("CLI emits complete Web application assets", async () => {
  ...
  assert.deepEqual(manifest.compiler, { name: "velar", version: "0.10.0" });
