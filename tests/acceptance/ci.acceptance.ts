...
test("default CI runs the complete gate while rehearsal and npm publication remain explicit", async () => {
  ...
  assert.match(externalPreview, /build examples\/app --out-dir release\/external-preview\/site/u);
...
test("the quick Node suite is everything that is not slow", async () => {
  // D115 P5: the quick suite no longer excludes by history. What it defers is
  // written in the file name, so the two claims worth pinning are that the
  // suffix is the *only* thing it defers and that nothing else quietly fell out.
  ...
  for (const required of [
    "compiler/analysis/collections.test.ts",
    "acceptance/ci.acceptance.ts",
    "acceptance/release.acceptance.ts",
    "web/live-props.test.ts",
  ]) {
    assert.ok(quick.includes(required), `the quick Node suite lost ${required}`);
  }
  assert.equal(quick.some((file) => file.endsWith(".slow.test.ts")), false, "a slow test ran in the quick suite");
  assert.deepEqual(full.filter((file) => !quick.includes(file)).filter((file) => !file.endsWith(".slow.test.ts")), []);
...
test("[D61-156] the test gates discover example projects instead of naming them", async () => {
  ...
  const browserAcceptance = await readFile(resolve(repositoryRoot, "tests", "acceptance", "browser.acceptance.ts"), "utf8");
  assert.match(browserAcceptance, /from "playwright"/u);
  assert.doesNotMatch(browserAcceptance, /\bfirefox\b|\bwebkit\b/u);

  const installedBrowser = await readFile(resolve(repositoryRoot, "tests", "acceptance", "installed-browser.acceptance.ts"), "utf8");
