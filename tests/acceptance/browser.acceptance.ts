...
import { chromium, type Browser, type BrowserServer, type BrowserType, type Page } from "playwright";
import {
...
async function runBrowserAcceptance(): Promise<void> {
  ...
  try {
    const scenario = async (): Promise<void> => {
      ...
      assert.equal(status.apiVersion, "0.14");
...
async function acceptBrowser(
  ...
  try {
    ...
    const { page, failures, frozenReadWarnings, uploads } = await openInstrumentedPage(browser);
    await acceptFirstRender(page, baseUrl, production);
    await acceptRuntimeCapabilities(page, uploads);
    await acceptRealtimeAndTransfers(page, production, realtimePort);
    await acceptFrozenReadReport(page, production, frozenReadWarnings);
    await acceptFormAndNavigation(page);
...
/** What one scenario is watched through: the page, and the three collectors its assertions read. */
interface InstrumentedPage {
  readonly page: Page;
  readonly failures: string[];
  readonly frozenReadWarnings: string[];
  readonly uploads: { requests: number };
}

/**
 * Setup: a permissioned context, a page, and the instrumentation every phase
 * below reads back — the upload route that stands in for the server, the
 * failure collector, and the frozen-read warnings D70 rule 180 expects.
 */
async function openInstrumentedPage(browser: Browser): Promise<InstrumentedPage> {
  ...
  // design, which `tests/acceptance/ci.acceptance.ts` asserts by refusing to find any
  // other engine named here.
  const context = await browser.newContext({ permissions: ["clipboard-read", "clipboard-write"] });
  const page = await context.newPage();
  const failures: string[] = [];
  const uploads = { requests: 0 };
  ...
  return { page, failures, frozenReadWarnings, uploads };
}

/** The first load: the document the build produced, and the state it reaches on its own. */
async function acceptFirstRender(page: Page, baseUrl: string, production: boolean): Promise<void> {
...
/** The capabilities a person drives: events, recovery, the worker, the file API, the clipboard. */
async function acceptRuntimeCapabilities(page: Page, uploads: { requests: number }): Promise<void> {
  ...
  assert.equal(uploads.requests, 1);
...
/** The two transports a page reaches out through, and the file it hands back. */
async function acceptRealtimeAndTransfers(page: Page, production: boolean, realtimePort: number): Promise<void> {
  if (!production) {
...
/** D70 rules 179/180, which is a development-mode report and a production silence. */
async function acceptFrozenReadReport(page: Page, production: boolean, frozenReadWarnings: readonly string[]): Promise<void> {
...
/** The form's three verdicts, then the route change and the reload that must survive it. */
async function acceptFormAndNavigation(page: Page): Promise<void> {
