...
try {
  ...
  assert.equal(installedManifest.dependencies["@velarscript/compiler"], "0.33.1");
  assert.equal(installedManifest.dependencies["@velarscript/core"], "0.33.1");
  assert.equal(installedManifest.dependencies["@velarscript/node"], "0.33.1");
  assert.equal(installedManifest.dependencies["create-velar"], "0.33.1");
  ...
  for (const target of ["@velarscript/web", "@velarscript/server", "@velarscript/desktop"]) {
    ...
    assert.equal(installedManifest.peerDependencies?.[target], "0.33.1", `CLI must pin the exact ${target} peer`);
  ...
  assert.equal(installedNodeManifest.dependencies["@velarscript/compiler"], "0.33.1");
  ...
  assert.deepEqual(installedServerManifest.velar.extension, {
    ...
    apiVersion: "0.15",
    manifestKey: "server",
    composes: {"@velarscript/node": "0.17"},
  });
  assert.equal(installedServerManifest.dependencies["@velarscript/compiler"], "0.33.1");
  assert.equal(installedServerManifest.dependencies["@velarscript/node"], "0.33.1");
  ...
  assert.equal(installedWebManifest.velar?.extension?.apiVersion, "0.14");
  ...
  assert.equal(versionLine, "velar 0.33.1");
...
`.trimStart(), "utf8");
  ...
  const nodeRuntime = await run(process.execPath, [
    "--input-type=module",
    "--eval",
    "import {VELAR_NODE_API_VERSION,VELAR_NODE_MODULES,velarNodeRuntime} from '@velarscript/node'; import {nodeModuleSources,velarNodeCompilerExtension} from '@velarscript/node/compiler'; const fs=nodeModuleSources.get('velar/fs') ?? ''; const http=nodeModuleSources.get('velar/http') ?? ''; const serve=nodeModuleSources.get('velar/serve') ?? ''; const websocket=nodeModuleSources.get('velar/websocket') ?? ''; if (VELAR_NODE_API_VERSION !== '0.17' || VELAR_NODE_MODULES.length !== 11 || velarNodeRuntime.name !== '@velarscript/node' || velarNodeCompilerExtension.id !== '@velarscript/node' || nodeModuleSources.has('velar/hash') || !fs.includes('export async function readText') || !fs.includes('export async function createText') || !fs.includes('export async function replaceTextIfMatches') || !fs.includes('export async function watchFiles') || !fs.includes('export const FileWatcher') || !fs.includes('__velarNodeHostInvoke(\"fs.createFile\"') || !fs.includes('__velarNodeHostInvoke(\"fs.replaceFileIfMatches\"') || !fs.includes('__velarNodeHostInvoke(\"fs.watchNext\"') || !http.includes('streamText') || !http.includes('__velarAssertJson') || !http.includes('__velarJsonStringify') || !http.includes('__velarNodeHostInvoke(\"http.request\"') || !http.includes('HttpTransportError') || !http.includes('HttpTransportPhase') || !http.includes('maxResponseChunks') || !serve.includes('parse: async (Type') || !serve.includes('ServeRequest.parse') || !serve.includes('export async function run') || !websocket.includes('export async function listen') || !websocket.includes('export async function run') || !websocket.includes('closeInfo()') || !nodeModuleSources.get('velar/server-test')?.includes('export async function client') || !nodeModuleSources.get('velar/terminal')?.includes('readLine')) process.exit(1); console.log(velarNodeRuntime.modules.join(','))",
  ], directory);
  assert.equal(nodeRuntime.stdout, "velar/server-test,velar/serve,velar/fs,velar/env,velar/host,velar/terminal,velar/path,velar/process,velar/http,velar/worker,velar/websocket\n");

  const serverRuntime = await run(process.execPath, [
    "--input-type=module",
    "--eval",
    "import {VELAR_SERVER_API_VERSION,VELAR_SERVER_MODULES,velarServerFramework} from '@velarscript/server'; import {serverModuleSources,velarCompilerExtension} from '@velarscript/server/compiler'; const configured=velarCompilerExtension.modules?.source?.('velar/server', {configuration:'config/server.yml'}) ?? ''; const server=serverModuleSources.get('velar/server') ?? ''; const realtime=serverModuleSources.get('velar/realtime') ?? ''; if (VELAR_SERVER_API_VERSION !== '0.15' || VELAR_SERVER_MODULES.length !== 2 || velarServerFramework.name !== '@velarscript/server' || velarCompilerExtension.contract?.kind !== 'application' || velarCompilerExtension.contract?.composes?.['@velarscript/node'] !== '0.17' || !configured.includes('applicationConfigurationPath = \"config/server.yml\"') || !server.includes('applicationConfigurationPath = \"\"') || server.includes('applicationConfigurationPath = \"application.yml\"') || !server.includes('export async function application') || !server.includes('export function authenticate') || !server.includes('export function database') || server.includes('export async function realtimeSession') || !realtime.includes('export async function realtimeSession')) process.exit(1); console.log(VELAR_SERVER_MODULES.join(','))",
  ], directory);
  ...
  const framework = await run(process.execPath, [
    "--input-type=module",
    "--eval",
    "import {VELAR_WEB_API_VERSION, VELAR_WEB_MODULES, velarWebFramework, webModuleSource} from '@velarscript/web'; if (VELAR_WEB_API_VERSION !== '0.14' || VELAR_WEB_MODULES.length !== 13 || velarWebFramework.name !== '@velarscript/web' || !webModuleSource('velar/web')?.includes('export function domId') || !webModuleSource('velar/look')?.includes('export function rgb') || !webModuleSource('velar/worker')?.includes('export function worker(') || !webModuleSource('velar/websocket')?.includes('export function connect') || !webModuleSource('velar/realtime')?.includes('export function realtimeClient')) process.exit(1); console.log(velarWebFramework.modules.join(','))",
  ], directory);
...
`.trimStart(), "utf8");
  ...
  assert.equal(docsManifest.dependencies["@velarscript/web"], "0.33.1");
  ...
  assert.equal(componentManifest.peerDependencies["@velarscript/web"], "^0.33.1");
  ...
  assert.equal(nodeManifest.dependencies["@velarscript/server"], "0.33.1");
  ...
  assert.equal(createdDesktopManifest.dependencies["@velarscript/desktop"], "0.33.1");
