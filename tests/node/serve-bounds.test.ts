...
test("the emitted velar/serve runtime carries each hardening contract", () => {
  ...
  // `code:` became `reason:` when the Node problem document took the spelling
  // `tests/node/node-http-problem-reason.test.ts` pins. This file was deferred
  // to `test:full` at the time and nobody ran it, so the assertion kept naming a
  // field the runtime no longer writes — which is the whole reason D115 P5
  // stopped deferring tests by the wave that wrote them.
