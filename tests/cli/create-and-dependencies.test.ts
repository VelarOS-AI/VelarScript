...
test("CLI creates explicit format-v2 projects and rejects legacy manifests without overwriting user files", async () => {
  ...
  assert.equal(createdPackage.dependencies["@velarscript/web"], "0.33.0");
  assert.equal(createdPackage.devDependencies["@velarscript/cli"], "0.33.0");
  ...
  assert.equal(componentPackage.peerDependencies["@velarscript/web"], "^0.33.0");
  assert.equal(componentPackage.devDependencies["@velarscript/web"], "0.33.0");
  ...
  assert.equal(nodePackage.dependencies["@velarscript/server"], "0.33.0");
  ...
  assert.equal(desktopPackage.dependencies["@velarscript/desktop"], "0.33.0");
...
test("CLI help is command-specific and malformed top-level invocations fail cleanly", () => {
  ...
  assert.equal(creatorVersion.stdout, "create-velar 0.33.0\n");
