...
const linkWebExtension = (root: string): Promise<void> => linkVelarExtension(root, "web");
...
`,
  ...
  const served = await fetch(`http://127.0.0.1:${port}${target}`);
  ...
  for (const engine of [chromium, firefox, webkit]) {
    assert.equal(await renderedText(engine, `http://127.0.0.1:${port}/`, "[data-label]"), "HIGHLIGHTED:hello");
  }
});
...
`,
  ...
  assert.equal(await renderedText(chromium, `http://127.0.0.1:${port}/`, "[data-label]"), "DEEP:library");
});
