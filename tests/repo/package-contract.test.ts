...
test("[A-024] workspace acceptance walks all derived package layers", async () => {
  ...
  const acceptance = await readFile(join(root, "tests", "acceptance", "package.acceptance.ts"), "utf8");
...
test("[A-024] every acceptance script that installs the toolchain derives its set", async () => {
  ...
  for (const name of ["acceptance/package.acceptance.ts", "acceptance/installed-browser.acceptance.ts"]) {
