...
try {
  ...
  assert.equal(installedManifest.dependencies["@velarscript/compiler"], "0.27.2");
  assert.equal(installedManifest.dependencies["@velarscript/core"], "0.27.2");
  assert.equal(installedManifest.dependencies["@velarscript/node"], "0.27.2");
  assert.equal(installedManifest.dependencies["create-velar"], "0.27.2");
  ...
  for (const target of ["@velarscript/web", "@velarscript/server", "@velarscript/desktop"]) {
    ...
    assert.equal(installedManifest.peerDependencies?.[target], "0.27.2", `CLI must pin the exact ${target} peer`);
  ...
  assert.equal(installedNodeManifest.dependencies["@velarscript/compiler"], "0.27.2");
  ...
  assert.equal(installedServerManifest.dependencies["@velarscript/compiler"], "0.27.2");
  assert.equal(installedServerManifest.dependencies["@velarscript/node"], "0.27.2");
  ...
  for (const dependency of ["@velarscript/compiler", "@velarscript/node", "@velarscript/web"]) {
    assert.equal(installedDesktopManifest.dependencies[dependency], "0.27.2");
  }
  ...
  assert.equal(versionLine, "velar 0.27.2");
...
`.trimStart(), "utf8");
  ...
  assert.equal(docsManifest.dependencies["@velarscript/web"], "0.27.2");
  ...
  assert.equal(componentManifest.peerDependencies["@velarscript/web"], "^0.27.2");
  ...
  assert.equal(nodeManifest.dependencies["@velarscript/server"], "0.27.2");
  ...
  assert.equal(createdDesktopManifest.dependencies["@velarscript/desktop"], "0.27.2");
