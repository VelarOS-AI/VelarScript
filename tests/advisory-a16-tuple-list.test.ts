...
/**
 * D114 ⑤ — A16: a heterogeneous List literal written where Python would write
...
test("A16 fires on an unannotated binding and names the record spelling", () => {
  const reported = advisories('const pair = ["a", 1]\nprint(str(pair.size))\n');
  assert.deepEqual(reported.map((item) => item.code), ["A16"]);
...
test("A16 carries no mechanical fix, because the field names are a judgement", () => {
  assert.deepEqual(advisories('const pair = ["a", 1]\nprint(str(pair.size))\n').map((item) => item.fix), [undefined]);
});

test("A16 fires on a body-inferred return, an arrow body, and an enum beside a string", () => {
  ...
  assert.deepEqual(inferred.map((item) => item.code), ["A16"]);
  ...
  assert.deepEqual(mapped.map((item) => item.code), ["A16"]);
  ...
  assert.deepEqual(mixedEnum.map((item) => item.code), ["A16"]);
  assert.equal(mixedEnum[0]!.written, '[Status.a, "x"]');
});

test("A16 stays silent wherever a position declared the element type", () => {
...
test("A16 stays silent on one element type, on null items, on two enums, and on records", () => {
...
test("A16 stays silent when any item is outside the primitive categories", () => {
  assert.deepEqual(codes([
  ...
  assert.deepEqual(codes([
    'const known = ["a", 1]   // velar-allow A16: proven separately',
...
test("A16 stays silent on an empty and on a single-element literal", () => {
...
test("a reasoned velar-allow A16 silences the line, and a stale one is a compile error", () => {
  const suppressed = compile(
    `${declarations}const pair = ["a", 1]   // velar-allow A16: the wire format is a positional pair\nprint(str(pair.size))\n`,
  );
  ...
  const stale = compile(
    `${declarations}const pair = ["a", "b"]   // velar-allow A16: not a tuple at all\nprint(str(pair.size))\n`,
  );
  assert.deepEqual(stale.diagnostics.map((item) => item.code), ["VEL1012"]);
  assert.match(stale.diagnostics[0]!.message, /No A16 advisory is reported on this line/u);
});

test("A16 never blocks emission", () => {
...
test("'velar check' exits 0 on a module whose only report is A16 and names the count", async (t) => {
  ...
  assert.match(`${stdout}${stderr}`, /advisory A16: A List holds one element type/u);
